      ...
      <div class="viewList" v-for="(item, index) in list" :key="index" @click="jumpDetail(item.id)" @mouseenter="enterProduct(item.id)">
        ...
        <div class="textArea">
          <div>
            ...
            <count @cutCount="cutCount" @addCount="addCount" :sigle-price="item.price"/>
          </div>
...
export default {
  ...
  computed: {
    ...
    totalMoney() {
      return this.$store.state.common.totalMoney
    }
  },
  watch: {
    scrollState(){
  ...
  methods: {
    addCount(val) {
      console.log(val)
      this.$store.dispatch('changeTotalMoney', {'flag': true, 'money': val})
    },
    cutCount(val) {
      console.log(val)
      this.$store.dispatch('changeTotalMoney', {'flag': false, 'money': val})
    },
    enterProduct(id) {
      this.productNum = id
    },
...
.goPay{ position: fixed;bottom: 5px;left: 5%;display: inline-block;width: 90%;height: 100px; background-color: white;box-shadow: 0 10px 10px #ccc; 
  .pay{ width: 100px;height: 100%;line-height: 100px;float: right;}
  .numDesc{ float: left;}
  .totalNum{ font-size: 40px;float: left;}
  .shopping-cart{ float: left; width: 90px;height: 90px;margin-top: 5px;margin-left: 5px;background-image: url("/static/image/foodList/shopping.png")}
}
