      ...
      <div class="viewList" v-for="(item, index) in list" :key="index" @click="jumpDetail(item.id)">
        ...
        <div class="textArea">
          <div>
            ...
            <count @cutCount="cutCount" @addCount="addCount" :sigle-price="item.price" :sigle-id="item.id"/>
          </div>
  ...
  methods: {
    addCount(val, id, num) {
      ...
      if(!this.payListArr.includes(id)) {
        ...
        const single = this.payList.find(ele => ele == id)
        console.log(single, '---------')
      }
      console.log(this.payList, this.payListArr)
    ...
    cutCount(val, id, num) {
      console.log(val, id)
      if(num == 0){
        const id = this.payList.indexOf(id)
        this.payList.splice(id, 1)
      }
      console.log(this.payList)
      this.totalMoney = Number(this.totalMoney) - Number(val)
    },
    gotoPay() {
      this.$router.push({path: '/pay', query: {info: this.payList}})
    },
    jumpDetail(id) {
