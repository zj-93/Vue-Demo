...
export default {
  ...
  data () {
    return {
      ...
      totalMoney: this.$store.state.common.totalMoney,
      payList: this.$store.state.common.payList,
      payListArr: this.$store.state.common.payListArr,
    };
  },
  computed: {
    scrollState() {
  ...
  methods: {
    addCount(val, id, num) {
      ...
      this.$store.dispatch('changePayListArr', this.payListArr)
      this.totalMoney = Number(this.totalMoney) + Number(val)
      this.$store.dispatch('changeTotalMoney', this.totalMoney)
    },
    cutCount(val, id, num) {
      console.log(val, id)
      if(num == 0){
